...
| The network path of the crawl adapter (`src/crawl/spider_engine.rs`) is what no test reaches, since a test may not use the network. A change to how the engine is configured compiles and passes `bin/ci` while being broken. `cargo run --example capture_seed -- <url> <dir>` against a server on localhost is the check. What the example still reaches and no test does is a crawl of more than one page, and the queue between the engine and the archive under real timing. | tripwire | partial: `tests/fetch_hardening.rs` drives the redirect guard and `tests/asset_capture.rs` the single fetch, each against a server it starts on loopback |
...
| The readability scoring pass is cubic in nesting depth, so a 40 KB page of nested `<div>` costs minutes of CPU, and neither the byte ceiling nor the element ceiling catches it. `MAX_NESTING_DEPTH` is the only thing that does, and its value is not pinned by anything: the test refuses a page nested 5000 deep, so raising the ceiling to 4000 keeps the suite green while restoring the whole cost. | tripwire | partial: `markup_nested_deeply_enough_to_be_a_weapon_is_refused` proves a guard exists, not that its value is safe |
