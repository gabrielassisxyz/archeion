...
| The two paths that write an article disagree about what a captured page may put in one, and the weaker one is the older one. Reading `readability/served.rs` and assuming the HTML path does the same is the mistake, in two places. Destinations: the served path allows `http`, `https`, `mailto` and a fragment and drops the rest, where the HTML path allows whatever its scorer leaves behind, which measures as dropping a destination spelled `javascript:` in lower case and keeping `JaVaScRiPt:`, `vbscript:` and `data:text/html,...`. Image descriptions: the served path reduces one to something that can only be a description, where the HTML path writes a page's `alt` into `![…](…)` almost verbatim, so `alt="x&#10;# Injected&#10;y"` writes a heading into the stored article at column zero and `alt="a](javascript:alert(1))"` writes a destination nothing screened. Both are pre-existing, and closing either changes the output of extractions already stored, which makes it a migration rather than a fix made in passing. | tripwire | partial: `a_destination_that_exists_to_run_loses_its_link_and_keeps_its_text` and the two `an_image_description_cannot_...` tests in `readability/served.rs` cover the served path, and nothing covers the other |
