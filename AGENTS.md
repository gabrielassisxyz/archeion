...
| The crawl engine reads a page limit or a depth of zero as no limit at all, so a seed built with either at zero crawls a site without a bound and nothing in the run says it was meant to stop. The command line refuses both, and a caller building a `Seed` by hand gets no such refusal. | tripwire | partial: `a_limit_of_zero_is_refused_rather_than_read_as_no_limit` in `src/cli/capture.rs` covers the command line only |
