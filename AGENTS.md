...
| An href written with a character reference for `&` (`&amp;`, `&#38;` or `&#x26;`) is resolved into a URL by `push_link` before anything decodes it, so a fetch aimed at the href as the page wrote it used to ask for the escape verbatim or, for the two numeric spellings, for a query truncated at the `#` a URL parser reads out of them. `arch-42q` closed the request half of this: `hop_depth_guard` resolves the same href a second time, decoded, whenever decoding changes it, and `rewrite_escaped_href`'s `on_link_find_callback` hands the engine that address in place of the one it resolved, so the request that goes out asks for what the page meant and `crawl_seed`'s own depth bookkeeping is keyed on the same corrected spelling rather than the one nobody requested. What stays open is the shape the real 30-page probe of `parknotes.substack.com` found and this does not reach: the same page linked from two different referring pages under two different spellings is still two entries in the engine's own frontier, since `push_link`'s dedup runs on the undecoded string before either request is resolved, and rewriting the frontier itself is the fix this project has already declined elsewhere (`docs/crawl-boundary.md`). Canonicalization still folds both into one archived item, so no link is lost, but the run's page budget is still spent twice on it. | tripwire | partial: `an_escaped_ampersand_between_parameters_is_still_a_separator` in `src/canonical_url.rs` covers the address; `an_escaped_ampersand_in_the_query_separator_reaches_the_server_decoded`, `a_decimal_character_reference_in_the_query_separator_reaches_the_server_decoded` and `a_hex_character_reference_in_the_query_separator_reaches_the_server_decoded` in `tests/cli_capture.rs` cover the request for a single link; nothing covers the same page linked under two different spellings from two different referring pages |
...
| `SpiderEngine` remembers the last `Website` it configured, keyed by origin, across every `crawl()` call made on the same instance, which is what lets a sitemap phase's per-URL sub-crawls sharing a host skip asking that host for `robots.txt` a second time: the crate's own `configure_robots_parser` no-ops its fetch once a parser's `mtime` is set. That reuse is why `configure_for_seed` has to reapply every setting on every call rather than lean on `Website::new`'s defaults, and why `robots_rules` has to rederive `Crawl-delay` from the parser on every call rather than trust the website's own delay field: the crate only writes that field from the parser the one time it actually fetches, so a reused sub-crawl that skipped `configure_for_seed`'s reset-then-reread would run under the operator's `--delay` instead of the site's from its second listed URL on. A `SpiderEngine::default()` built fresh per seed, rather than reused across a run, throws the cache away silently and returns to asking for `robots.txt` once per seed. | tripwire | partial: `a_sitemap_run_with_an_explicit_depth_reads_robots_txt_once_for_every_listed_url` and `a_url_the_reused_engine_s_robots_file_disallows_is_still_refused` in `tests/sitemap.rs` prove the reuse and the disallow decision end to end against a real host, and `a_reused_sub_crawl_still_honours_the_site_s_crawl_delay` in `src/crawl/spider_engine.rs` pins the delay directly; nothing pins that every other setting in `configure_for_seed` is still reapplied on a reused instance, which is proven by reading the chain rather than by a test built for it |
...
| `ReferencedAsset.fallback` (`src/metadata/model.rs`), the address a repass tries when a `srcset`'s widest candidate misses for a reason `retryable_miss` answers false for, is only ever read from the metadata beside the capture, never recomputed from the stored body. Adding the field did not move `EXTRACTOR_VERSION`, on the same reasoning version 4's own comment states for an added field: an old record's absence of it already reads correctly as "this extractor did not look for it". So an archive captured and extracted before this landed has no `fallback` to read, and running a repass over it does not, by itself, retroactively compute one; only a re-extraction triggered some other way, a site rule added or the version moved again for an unrelated reason, would. `Asset.is_fallback` (`src/storage/model.rs`) has the same shape on the capture side: it is written by the run that recovers the rendition, `#[serde(default)]` reads an older record as `false`, and nothing walks an existing archive to fill it in. | tripwire | yes: `a_widest_candidate_missed_for_good_is_recovered_from_its_fallback_rendition` in `tests/repass.rs` proves the recovery once the metadata carries the field; nothing covers a repass over a capture whose metadata predates it |
