...
| The two paths that write an article disagree about what a link may point at, and the weaker one is the older one. The served-Markdown path allows `http`, `https`, `mailto` and a fragment and drops the rest. The HTML path allows whatever its scorer leaves behind, which measures as dropping a destination spelled `javascript:` in lower case and keeping `JaVaScRiPt:`, `vbscript:` and `data:text/html,...`. Reading the served code and assuming the HTML path does the same is the mistake here. | tripwire | partial: `a_destination_that_exists_to_run_loses_its_link_and_keeps_its_text` in `readability/served.rs` covers the served path, and nothing covers the other |
