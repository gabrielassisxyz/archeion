...
Four of them, all per capture except the last, which is the run's.
...
- **Thirty-two megabytes for all of one capture's subresources.** This is where the pass stops asking, not a total it guarantees: the size of the next subresource is unknown until it arrives, so the last one can cross the line, and the overshoot is bounded by what one subresource may spend.
...
- **Before it is followed.** The scheme has to be http or https, and the address may not be one that exists only inside a network: loopback, a private range, link-local, or a name a cloud metadata service answers on. A run that asked for local addresses gets them, which is how a locally served site is archived at all.
...
Every reference that did not become a subresource is in the capture record with its reason: no response, too large, arrived short, a ceiling reached, the deadline gone, or an address inside a network.
