...
A later repass may ask again for subresources the capture already recorded as missed by archive policy, such as a count ceiling, a byte ceiling or a deadline. It does not crawl the page again and it does not blindly retry a URL that already got a direct no-response result. The order above governs a repass too, and the role each reference was named in is read back from the metadata record beside the capture: a repass that handed every retry over as an image had a uniform priority, so a capture with more retryable misses than one pass deals with bought back whatever its record happened to list first. Recovered assets and retry results that are still missing are written beside the capture in `<capture-id>.assets-recovered.json`, because rewriting the capture record would make its id stop describing the fields it names.
...
- **A hundred and twenty-eight references dealt with.** It counts references dealt with and not files kept, and that difference is the whole bound. A page listing two thousand addresses that answer nothing stores nothing, so a ceiling on what is stored would never be reached, and the run would spend two thousand requests on somebody else's behalf learning that. Whoever wrote the page picks those addresses. A reference the run has already answered is not one of them and is not counted: the answer is served before this ceiling is consulted at all, since a memo hit is not a request and refusing one bounded nothing while making the record report an absence over bytes the archive was holding. What the ceiling still bounds is what it was drawn against, a page of addresses nobody has answered, because the reference list is deduplicated by address and a page cannot spend the budget on repeats of one URL. It is therefore not also a ceiling on how many subresources one capture holds: a page naming addresses the run already learned references every one of them, bounded by the two thousand references extraction hands over at all, and costing nothing in the store, where the bodies already sit under their own hashes.
...
- **What the request carries.** A run may hold a session, and that credential is bound to one origin: a subresource on the same origin as the page is asked for with it, and one on a content network is asked for without it, which is where most pictures live. The reference is a remote document choosing an address, so a rule that attached the session to whatever it named would let a page hand the operator's subscription to a host of its choosing. [`crawl-boundary.md`](crawl-boundary.md) has the binding and the one half of it that belongs to the HTTP client.
...
The answer to a subresource is remembered for the whole run, including the answer that there is none, and it is served before any ceiling is consulted.
...
- **Concurrency.** The pass fetches one subresource at a time, which bounds what one page's subresources add to the load already on a host. It is not the run's politeness delay and must not be relied on as one, which is the correction a real run paid for: `--delay` did not reach the sitemap phase, that phase's only pacing was how long this pass happened to take, and when a change cut a page's pictures from eight requests to two the run went from 22.7 minutes to 1.9 and the host refused 160 of its 250 pages. The delay is now the sitemap phase's own, and a crawl's is the engine's. A page's subresources are still unpaced on both paths, deliberately: they are a bounded burst against a host the run is already talking to, not a queue of hundreds. The cost is real and worth stating exactly: because the pass blocks the callback pages arrive through, a run with subresources archives fewer pages inside the same deadline, and a run that meets slow subresources can have pages fetched and then dropped for want of anyone reading them. The bound above is what keeps the pathological case from eating a run; the ordinary case, tens of files answering in tens of milliseconds, fits inside the queue. Fetching a page's subresources in parallel would cut the block and multiply what the archive asks of a host at once, so it is a measurement to make against a real site rather than a guess to ship.
