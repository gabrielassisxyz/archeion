...
Each ceiling has a test that fails when the ceiling is written the plausible wrong way, counting files kept rather than references dealt with or bytes transferred, because both wrong versions pass every other test in this project while bounding nothing.
