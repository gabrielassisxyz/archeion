...
A 429 is the exception to moving straight on, because it says the address exists and only the pace is wrong: the run waits and asks again, growing the wait across a server's successive refusals and honouring a `Retry-After` the server sent. One address may spend a quarter of the budget on being waited out, and is recorded as owed once a further wait would cost more than that; the budget in question is what the phase still has rather than the flag's own number, because a sitemap phase given an explicit depth and every `--resume` ask for one address at a time under what is left of the deadline. When it is the growth itself that outran the share, rather than one response's `Retry-After`, the run stops waiting for that server altogether and records its remaining addresses as owed at the cost of one request each. A page recovered by waiting is an ordinary capture and owes nothing, and the report says how many there were: the `waited out` row, carried by `--json` as `pages_recovered_from_rate_limit`. A zero says nothing was got back by waiting, which a run that met no rate limit and a run that waited one out without success both produce; `host refused` beside it is what tells those two apart. [`crawl-boundary.md`](crawl-boundary.md) has the policy and its bounds.
