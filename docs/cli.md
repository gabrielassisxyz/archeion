...
`--progress` is the same flag with the same levels, spelled the same way: `--progress=bar`, `--progress=lines`, or bare for `lines`. It goes to stderr and stdout is unaffected, exactly as it is on `capture`. What `lines` names is one capture and what its derived records became, `written`, `article refused`, `not article`, `metadata written` or `unchanged`, and what `bar` counts is items of the archive against how many the walk found. Every item advances the bar, including one whose captures could not be read at all, so the bar reaches its own total rather than stopping short of it.
