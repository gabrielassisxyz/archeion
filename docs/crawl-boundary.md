...
**A lost page is counted, never inferred.** The queue between the engine and the archive is bounded: sized to the fetch concurrency alone it would drop pages as soon as a write is slower than a fetch, and sized to the page limit it would hold a whole crawl's bodies in memory. It is sized between the two, and what overflows anyway is counted into the outcome. A caller that breaks out early counts what it leaves queued the same way, since those pages cost a fetch each and the archive does not have them. Silently archiving less than was fetched is the failure this project can least afford.
